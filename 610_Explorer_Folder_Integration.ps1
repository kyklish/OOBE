...
"Adding [Open TotalCMD here] to context menu..."
...
function Set-ExplorerFolderContextMenuEx {
    ...
    Set-Item -Path $path -Value $Message
    New-ItemProperty -Path $path -Name "Icon" -Value $Exe | Out-Null
    $path = "$path\command"
    New-Item -Path $path -Force | Out-Null
    Set-Item -Path $path -Value $cmd
}
...
# Set-Item -Path $path -Value $msg
# New-ItemProperty -Path $path -Name "Icon" -Value $cmd
# $path = "$path\command"
# New-Item -Path $path -Force
# Set-Item -Path $path -Value "$cmd `"%V`""
...
# $path = "$path\command"
# New-Item -Path $path -Force
# Set-Item -Path $path -Value "$cmd `"%1`""
